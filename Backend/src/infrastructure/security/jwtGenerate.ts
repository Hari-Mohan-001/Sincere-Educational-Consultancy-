...
export const generateJwtToken = (res: Response, userId: string) => {
  const accessToken = jwt.sign({ userId,role:"user" }, accessTokenSecret, { expiresIn: "30m" });
...
export const generateRefershToken = (userId:string)=>{
  ...
  const refreshToken = jwt.sign({ userId }, refershTokenSecret, { expiresIn: "5d" });
  console.log('aftercre',refreshToken);
