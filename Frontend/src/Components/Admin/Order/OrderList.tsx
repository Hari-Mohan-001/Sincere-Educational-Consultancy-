...
const OrderList = () => {
  ...
  const columns = [
    ...
    { id: "createdAt", label: "Date", minWidth: 100, render:(row: Order) => formatDate(row.createdAt)},
    {
      id: "totalAmount",
      label: "Amount (Rs)",
      minWidth: 100,
  ...
  const downloadPDF = () => {
    ...
    if (startDate && endDate) {
      doc.setFontSize(11);
      doc.text(`Date Range: ${formatDate(startDate)} to ${formatDate(endDate)}`, 14, 25);
    }
    
    // Create the table
    (doc as any).autoTable({
      head: [columns.map(column => column.label)],
      body: orderData?.orders.map(order => [
        order.userName,
        order.userEmail,
        order.enrollType,
         "",// We can't include images easily in jspdf-autotable
        order.country,
    ...
    doc.text(`Total Order Value: Rs ${orderData?.grandTotalAmount || 0}`, 14, finalY + 10);
  ...
  return (
    ...
    <div className="mt-4 mr-2 ml-3">
      <Button variant="contained" onClick={downloadPDF}>Download Report</Button>
    </div>
  </div>
);
};
